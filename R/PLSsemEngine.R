...
#' @param file Character. The file path and name for the exported CSV. Must be
#'   supplied explicitly by the user (e.g. \code{file.path(tempdir(), "scores.csv")});
...
#'
#' @export
export_scores <- function(model, file = NULL) {

  if (is.null(file)) {
    stop(
      "Please specify a 'file' path explicitly (e.g. file.path(tempdir(), \"scores.csv\")). ",
      "export_scores() does not write to a default location.",
      call. = FALSE
    )
  }
...
#' @param file_name Character. Target file path for export. Required (no default
#'   location) when \code{save_plot = TRUE}, e.g. \code{file.path(tempdir(), "plot.png")}.
...
plot_structural_model <- function(structural_model, layout = NULL,
                                  ...
                                  box_col = "white",
                                  save_plot = FALSE, file_name = NULL,
                                  width = 2500, height = 1500, res = 300) {

  if (save_plot) {
    if (is.null(file_name)) {
      stop(
        "Please specify 'file_name' explicitly (e.g. file.path(tempdir(), \"plot.png\")) ",
        "when save_plot = TRUE.",
        call. = FALSE
      )
    }
